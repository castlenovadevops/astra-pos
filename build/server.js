...
app.use(function(request, response, next) {
  ...
  response.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept, api-key,udid,device-type,Authorization, accessToken");
  next();
});
...
io.on("connection", (socket) => {
  ...
  socket.on("resetSync", ()=>{
    console.log("RESET SYNCE SERVER")
    syncDataFlag = 0;
    socket.emit("stopSync", new Date());
  })
  socket.on("startSync", ()=>{
    console.log(syncDataFlag) ;
    if(syncDataFlag === 1){ 
      socket.emit("errorSync", {msg:"Sync is already in progress. Please wait."});
    }
    else{
      syncDataFlag = 1;
      console.log("START SYNC")
      socket.emit("startSync", new Date());
    }
  });
