...
module.exports = class WaitinglistController extends baseController{
    ...
    getAppointmentsList = async(req,res, next)=>{
        this.readAll({where:{
            ...
            this.updateApptStatus(0, r.dataValues || r, req, res, next)
        })
    }

    updateApptStatus = async(i, data, req,res, next)=>{
        if(i < data.length){
            this.update(`appointments`,{id:data[i].id, appointmentStatus:'Waiting', appointmentId: data[i].appointmentId}, {where:{
                appointmentDate:this.getDate().substring(0,10),
                appointmentStatus:'Booked', 
                appointmentId: data[i].appointmentId
            }}).then(r=>{
                this.updateApptStatus(i+1, data, req, res, next)
            })
        }
        else{
            this.sendWaitingList(req, res, next);
        }
    }

    sendWaitingList = async(req, res, next)=>{
